...
houseNorm <- house %>%
  mutate_at(c("bed", "fullBath", "halfBath", "age", "originalPrice", "lastPrice", "monthSold", "daySold", "yearSold", "marketDays"), ~(scale(.) %>% as.vector))
...
linearModel <- function(df, testSet) {
  lr <- lm(soldPrice ~ ., data = filter(df, id != testSet))
  summary(lr)
  predictions <- predict(lr, newdata = filter(df, id == testSet))
  eval_metrics(lr, filter(df, id == testSet), predictions, target = "soldPrice")
}

#linear regression with a subset of the predictors
#we use the step function for this

subsetModel <- function(df, testSet) {
